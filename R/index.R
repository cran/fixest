...
#' * Multiple estimations: You can perform multiple estimations at once with the [`stepwise`] functions. It's then very easy to manipulate multiple results with the associated methods. See an introduction in the dedicated vignette: [Multiple estimations](https://cran.r-project.org/package=fixest/vignettes/multiple_estimations.html)
#' * Easy and flexible clustering of standard-errors. By using the arguments `vcov` and `ssc` (see [`summary.fixest`]). To have a sense of how the standard errors are computed, see the vignette [On standard-errors](https://lrberge.github.io/fixest/articles/standard_errors.html).
#' * Visualization and exportation of results. You can visualize the results of multiple estimations in R, or export them in Latex using the function [`etable`]. This vignette details how to customize the Latex tables: [Exporting estimation tables](https://lrberge.github.io/fixest/articles/exporting_tables.html).
