...
So far (version 0.10.0), six types of standard-errors can be computed, here are their keywords and a brief description:
 ...
 - `iid`: assumes that errors are identically and independently distributed.
...
feols(y ~ x.[i] + .[z], base)
```
What did just happen? When `.[]` is encountered in a formula, its value is evaluated and inserted verbatim in the formula. Further, when the value in brackets is a vector, as many variables are added in the model. That's why `x.[i]` led to `x2 + x3`. Of course, numbers can be used directly: `x.[2:3]` would have led to the same result.

Variable names in which `.[]` is in the middle of the name can also be created, but the use of macros is then mandatory:
```{r}
i = 1:5
xpd(y ~ ..x, ..x = "x.[i]_sq")
```
The use of a character string is mandatory here to avoid formula parsing problems.
